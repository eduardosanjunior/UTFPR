...
/**
 * Tabuleiro de 4 x 4: o objetivo é dispor os números de 1 a 8 nas posicoes
 * indicadas sem que antecessores/sucessores tenham posicoes adjacentes nas
 * linhas, colunas e diagonal. As posicoes com X nao sao utilizadas.
 *
 * X 1 2 X
 * 3 4 5 6
 * X 7 8 X
...
public class Tabuleiro {
    ...
    /**
     * armazena o valor da fun objetivo (avaliação) para a posicao atual das
     * pedra
     */
    public int vlrObj;
    ...
    /**
     * Geração de vizinhos: cria 1 (um) estado sucessor pela permutacao de duas
     * pedras sorteadas aleatoriamente.
     *
    ...
    public void gerarVizinho(Tabuleiro atual) {
        ...
        int aux[][] = new int[1][1];
        aux[0][0] = this.pedra[a][b];
        this.pedra[a][b] = atual.pedra[c][d];
        this.pedra[c][d] = aux[0][0];
...
//        this.imprimirTabuleiro();
        for (int a = 0; a < pedra.length; a++) {
            for (int b = 0; b < pedra[0].length; b++) {
//                System.out.println("\n* * * a = " + a +" b = " + b);
                for (int i = -1; i < 2; i++) {
                    for (int j = -1; j < 2; j++) {
                        if ((a + i >= 0) && (a + i < pedra.length) && (b + j >= 0) && (b + j < pedra.length)) {
//                            System.out.println("a+i = " + (a+i) + " b+j = " + (b+j));
                            if (pedra[a][b] != pedra[a + i][b + j]) {
                                if (pedra[a][b] != 0) {
                                    if ((pedra[a + i][b + j] == pedra[a][b] + 1) || (pedra[a + i][b + j] == pedra[a][b] - 1)) {
                                        this.vlrObj++;
                                    }
                                }
                            }
     ...
     *
     * @return String o tabuleiro no formato CSV na ordem: primeira, segunda e
     * terceira linha (inclusive com as casas não ocupadas)
     */
