...
public class AGMochila {
     ...
     */
    private static final int MAX_EXECUCOES = 10000;
    /**
    ...
    public static void main(String[] args) {
	    Mochila best;
	    int ct = 0;
	    int mochilasMaximas = 0 ;
	    String mochilaCSV = null;
	    do {
	        AGMochila ag = new AGMochila();
	        best = ag.executarAG();
	        if (best != null) {
	        	/*if(best.Valor() == 206 && mochilaCSV != null) {
	        		if(mochilaCSV.compareTo(best.imprimirMochilaCSV()) != 0) {
	        			System.out.println(mochilaCSV);
	        			System.out.println(best.imprimirMochilaCSV());
	        			break;
	        		}
	        	}*/
	        	//if(best.Valor() == 206 && mochilaCSV == null) { mochilaCSV = best.imprimirMochilaCSV(); }
	            System.out.println(best.imprimirMochilaCSV());
	            best.imprimirMochila();
	        }
	        ct++;
	        System.out.println("Total de execucoes = " + ct);
	        if(best.Valor() == 206) { mochilasMaximas++; }
	    } while (ct < MAX_EXECUCOES);
	    //System.out.println("Numero de mochilas maximas: " + mochilasMaximas);
	}
}
